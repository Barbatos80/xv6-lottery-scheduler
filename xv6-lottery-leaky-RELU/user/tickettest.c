...
#define NCH 5
static int tvals[NCH] = {10,20,30,40,50};
static int pids[NCH];
...
main(void)
{
  ...
  for (int i=0;i<NCH;i++){
    ...
    if (pid == 0){
      ...
      if (settickets(tvals[i])<0){ printf(1,"settickets fail\n"); exit(); }
      burn();
      exit();
    }
    pids[i]=pid;
  ...
  // CSV header
  printf(1,"tickets,delta_ticks,share_pct\n");
